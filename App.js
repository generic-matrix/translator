import { StyleSheet, Text, ScrollView, View, TouchableOpacity,TextInput,ActivityIndicator} from 'react-native';
...
export default function App() {
  ...
  async function GetResult(){
    if(languageCode===undefined ||text.length===0){
      SetResult("Make sure to add the text to convert and select the language to convert to")
    }else{
      SetLoading(true);
      axios({
        method: 'get',
        ...
        SetResult("Bummer :( Error to translate , make sure the language is supported or intenet connection is available")
        SetLoading(false);
      });
    }
  }
  ...
  return (
   <ScrollView contentContainerStyle={styles.screen}>
      ...
      <Text style={styles.subtitle}>ML Kit Powered Translator</Text>
      ...
      <View>
        ...
        <Text style={styles.buttonText } onPress={GetResult}>Translate The Text</Text>
        </TouchableOpacity>
      </View>
      {
        (loading===false)?
        <Text style={styles.subtitle }>{result}</Text>:<ActivityIndicator size="large" />
      }
    </ScrollView>
  );
};
